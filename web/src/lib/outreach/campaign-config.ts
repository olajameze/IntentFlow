...
export function buildFollowUpPrompt(
  settings: BusinessOutreachSettings | null,
  campaign: string,
  prospect: { name: string; website_url?: string | null; sector?: string | null },
  touchIndex: number,
  tier: string,
  ...
  if (tier === "hot" && touchIndex === 0) {
    return {
      prompt: `Write a short follow-up (max 80 words) to ${prospect.name}. They clicked your booking/signup link but did not complete. Sector: ${angle}. Remind them the CTA button below is still open. One proof point. UK English. Sign off with team name only.`,
  ...
  return {
    prompt: `Write follow-up touch ${touchIndex + 1} for ${prospect.name} (${prospect.website_url || "no website"}). Sector angle: ${angle}. Tier: ${tier}. Max 90 words. UK English.`,
    fallbackSubject,
    fallbackBody,
  };
}
