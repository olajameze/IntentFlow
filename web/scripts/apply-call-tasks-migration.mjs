#!/usr/bin/env node
/**
 * Apply outreach call tasks migration (20260618000000_outreach_call_tasks.sql).
 * Uses SUPABASE_DB_URL when set; otherwise copies SQL and opens Supabase SQL Editor.
 *
 * Usage (from web/): node scripts/apply-call-tasks-migration.mjs
 */
...
const sqlPath = path.join(root, "..", "supabase", "migrations", "20260618000000_outreach_call_tasks.sql");
...
async function tableExists() {
  ...
  const { error } = await sb.from("outreach_call_tasks").select("id").limit(1);
  return !error;
}
...
async function main() {
  ...
  if (await tableExists()) {
    console.log("Call tasks migration already applied (outreach_call_tasks exists).");
    return;
  }
  ...
  if (dbUrl && !dbUrl.includes("YOUR_DB_PASSWORD")) {
    try {
      ...
      console.log("Call tasks migration applied via Postgres.");
      return;
    } catch (e) {
